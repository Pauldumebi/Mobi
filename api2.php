...
try {
    ...
    if (stripos($_SERVER['REQUEST_URI'], '/checkStatus')!== false) {
        ...
        $sql = "select phone, name, expires, network from mobi.users where phone = '$phone'";
        $query = mysqli_query($db, $sql);
        if (mysqli_num_rows($query) == 1) {
            $user = mysqli_fetch_array($query);
            $user = array('phone' => $user['phone'], 'network' => $user['network'],'name' => $user['name'], 'expires' => $user['expires']);
        }
        if (!$user)
            ...
            if ($resp) {
                if (stripos($resp, 'Subscription is being confirmed') >= 1 || stripos($resp, "The product has been subscribed to") >= 1 || stripos($resp, 'Temporary Order saved successfully!') >= 1) {
                    ...
                    if($emailsent) {
                        ...
                        $query = mysqli_query($db, $sql);
                        $user = mysqli_fetch_array($query);
                        $user = array('emailverified' => $user['email_verified']);
        ...
        if($db->errno)
            respond(500, array('success' => false, 'message' => 'db error: '.$db->error));
        $sql = "select phone, expires, network from mobi.users where phone = '$phone'";
        $query = mysqli_query($db, $sql);
        if (mysqli_num_rows($query) == 1) {
            $user = mysqli_fetch_array($query);
            $expiry_ok = date('Y-m-d H:i:s') < $user['expires'];
            $user = array('phone' => $user['phone'],'expires' => $user['expires'], 'network' => $user['network']);
            $emailsent = sendEmail ($email, $db);
            if($emailsent) {
                $sql = "select email_verified from mobi.users where phone = '$phone'";
                $query = mysqli_query($db, $sql);
                $emailVerified = mysqli_fetch_array($query);
                $emailVerified = array('emailverified' => $emailVerified['email_verified']);
            }
            respond(200, array('success' => true, 'message' => ['Profile Successfully Updated', $user, $emailVerified]));
        }
        else
        ...
        if ($email == $user) {
            ...
            respond(404, array('success' => false, 'error' => 'Sorry this email is not associated with this account'));
        }
        ...
        if (mysqli_num_rows($query) == 1) {
            ...
            if($db->errno)
                respond(500, array('success' => false, 'message' => 'db error: '.$db->error));
            respond(200, array('success' => true, 'message' => 'Email verified'));
        } else
        ...
        if($row){
            ...
            $query =  mysqli_query($db, $sql);
            respond(200, array('success' => true, 'message' => 'Password changed successfully'));
        } else 
